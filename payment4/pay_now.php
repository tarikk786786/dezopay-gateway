...
$sql_p = "SELECT * FROM bharatpe_tokens WHERE user_token='$user_token' AND phoneNumber = '$merchentMobile' AND status = 'Active' LIMIT 1";
...
//  $orders = "upi://pay?pa=$upi_id&am=$amount&pn=$unitId&tn=$cxrkalwaremark&tr=$cxrbytectxnref";
 $intd = "pa=$upi_id&am=$amount&pn=BharatPe Merchant&cu=INR&tn=Pay To BharatPe Merchant";
 $orders = "upi://pay?".$intd;
...
$paytmintent = "paytmmp://cash_wallet?pa=$upi_id&pn=BharatPe Merchant&am=$amount&cu=INR&tn=Pay To BharatPe Merchant&tr=$description&mc=4722&&sign=AAuN7izDWN5cb8A5scnUiNME+LkZqI2DWgkXlN1McoP6WZABa/KkFTiLvuPRP6/nWK8BPg/rPhb+u4QMrUEX10UsANTDbJaALcSM9b8Wk218X+55T/zOzb7xoiB+BcX8yYuYayELImXJHIgL/c7nkAnHrwUCmbM97nRbCVVRvU0ku3Tr&featuretype=money_transfer";
// Define the API endpoint
// Your custom QR code API URL
// $url = 'https://imbx.in/secret/create_qr.php';
$url = 'https://pay.dezo.in/secret/create_qr.php';
...
Swal.fire({
    ...
    preConfirm: (utr) => {
        ...
        return fetch('https://<?php echo $_SERVER["SERVER_NAME"] ?>/order4/payment-status', {
            method: 'POST',
            body: formData
        })
        .then(response => {
