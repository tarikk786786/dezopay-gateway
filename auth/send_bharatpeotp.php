...
// Step 1: Send OTP
function sendOtp() {
    fetch(`https://pay.dezo.in/auth/connectHelp/bharatpe.php?action=send&mobile=${mobile}`)
        .then(res => res.json())
        .then(data => {
...
function promptOtp(uuid, token) {
    Swal.fire({
        ...
        preConfirm: (otp) => {
            ...
            return fetch(`https://pay.dezo.in/auth/connectHelp/bharatpe.php?action=verify&uuid=${uuid}&otp=${otp}&mobile=${mobile}&token=${token}`)
                .then(res => res.json())
                .then(response => {
                    if (!response.success) {
