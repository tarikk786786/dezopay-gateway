...
// Step 1: Send OTP
function sendOtp() {
    fetch(`https://pay.dezo.in/auth/connectHelp/quintuspay.php?action=send&mobile=${mobile}`)
        .then(res => res.json())
        .then(data => {
...
function promptOtp(uuid) {
    Swal.fire({
        ...
        preConfirm: (otp) => {
            ...
            return fetch(`https://pay.dezo.in/auth/connectHelp/quintuspay.php?action=verify&otp=${otp}&mobile=${mobile}`)
                .then(res => res.json())
                .then(response => {
                    if (!response.success) {
